...
def one(INPUTS):
  ...
  loc = (0, 0)
  for edge in path_a:
    direction = dirs[edge[0]]
    length = int(edge[1:])
    new_loc = (loc[0]+direction[0]*length, loc[1] + direction[1]*length)
    if edge[0] in ["L", "R"]:
      row = edges_horiz.get(loc[1], [])
      row.append((loc[0], new_loc[0]))
      edges_horiz[loc[1]] = row
    else:
      col = edges_vert.get(loc[0], [])
      col.append((loc[1], new_loc[1]))
      edges_vert[loc[0]] = col
  ...
  for edge in path_b:
    ...
    for i in range(length):
      ...
      if edge[0] in ["L", "R"]:
        col = edges_vert.get(loc[0], [])
        for entry in col:
          if entry[0] < loc[1] < entry[1] or entry[1] < loc[1] < entry[0]:
            intersects.append(abs(loc[0])+abs(loc[1]))
      if edge[0] in ["U", "D"]:
        row = edges_horiz.get(loc[1], [])
        for entry in row:
          if entry[0] < loc[0] < entry[1] or entry[1] < loc[0] < entry[0]:
            intersects.append(abs(loc[0])+abs(loc[1]))
...
def two(INPUTS):
  ...
  for edge in path_a:
    ...
    for unused in range(length):
      ...
      row.append(loc[1])
      nodes[loc[0]] = row
      if loc not in dist_a: dist_a[loc] = i
      i += 1
