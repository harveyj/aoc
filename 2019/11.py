...
# 		self.puter = Intputer(instructions, inputs=[], id='a', input_mode='halt', output_mode='halt')
# 		self.loc = (0,0)
# 		self.dir = 0
# 		self.panels = {}
# 		self.trail = []

#   # black = 0; white = 1, start all black.
...
# 	def run(self):
# 		while not self.puter.terminated:
# 			code = self.puter.run()
# 			self.puter.halted = False
# 			if code == Intputer.OUTPUT:
# 				# print("OUTPUT", out)
# 				# print(self.puter.outputs)
# 				if len(self.puter.outputs) % 2 == 0:
# 					color, dir = self.puter.outputs[-2::]
# 					if dir == 1:
# 						dir_delt = 1
# 					else: dir_delt = -1
# 					self.update(color, dir_delt)
# 			elif code == Intputer.INPUT: 
# 				# print(self.panels, self.loc)
# 				print("INPUT", self.loc, self.panels.get(self.loc, 0))
...
def one(INPUT):
	return Painter(INPUT[0].split(','), []).run()

def two(INPUT):
	return 'DUMMY'
	painter = Painter(INPUT[0].split(','), [1]).run()
